...
@section('content')
    ...
    <div class="w-4/5 m-auto pt-20">
        <form action="{{route('blog.show', $post->slug)}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="text" name="title" class="bg-transparent block border-b-2 w-full h-20 text-6xl outline-none" value="{{ $post->title }}">
            <textarea name="description" class="w-full py-5 px-2 mt-4 bg-transparent block border-b-2 h-60 text-xl outline-none">
                {{ $post->description }}
            </textarea>
